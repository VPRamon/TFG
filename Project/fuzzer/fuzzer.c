...
struct modconfig *load_config(char* module_conf){
    ...
    int lvl = 0;
    FILE *fp = fopen("example_module.conf", "r");
    if (fp == NULL)
        exit(EXIT_FAILURE);
    size_t len = 0;
    char * line = NULL;
...
void report_bug(FILE **fd_read){
    FILE *fd_rep;
    if( (fd_rep = fopen("bug_report.txt", "a")) < 0) {
...
{   
    ...
    sprintf(buff, "\tint fd = open(\"/dev/%s\", O_WRONLY);\n", dev);
...
int main(){
    ...
    int num_of_cmd = modconf->ioctlcmds->len;
    
    char* dev = modconf->name;
    char magic = modconf->iocmagic;
    int syscall;
    int cmd;
    char *input;
    int count = 0;
